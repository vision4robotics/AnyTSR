...
def test_bicubic_resize_fn(img, l_size, h_size):
    return transforms.ToTensor()(
        transforms.Resize(h_size, InterpolationMode.BICUBIC)(
        transforms.Resize(l_size, InterpolationMode.BICUBIC)(
            transforms.ToPILImage()(img))))
...
class SRImplicitDownsampledFast(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.inp_size == 'bicubic':
            h_lr = math.floor(img.shape[-2] / s + 1e-9)
            w_lr = math.floor(img.shape[-1] / s + 1e-9)
            h_hr = round(h_lr * s)
            w_hr = round(w_lr * s)
            img = img[:, :h_hr, :w_hr]
            img_down = test_bicubic_resize_fn(img, (h_lr, w_lr),(h_hr, w_hr))
            crop_lr, crop_hr = img_down, img
        else:
            h_lr = self.inp_size
            w_lr = self.inp_size
